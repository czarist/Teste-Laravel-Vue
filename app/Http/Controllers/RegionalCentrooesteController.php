...
class RegionalCentrooesteController extends Controller
{
    ...
    public function formregionalcentrooeste()
    {
        $user = $this->usuario();
        $regiao = 4;

        return view('regionais.centrooeste.form', compact('user', 'regiao'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($now <= '2022-05-25 00:00:00') {
                ...
                $regional = RegionalCentrooeste::whereUserId($user->id)->first();

                if (empty($regional)) {
                    RegionalCentrooeste::create([
                        'user_id' => $user->id,
                        'regiao' => 'centrooeste',
                ...
                if (Auth::user()->anuidade_2022 && Auth::user()->is_associado) {
                    if (Auth::user()->pago_regional_centrooeste_2022 == false) {
                        $todos_tipos = [0 => 9];
                        $user->todos_tipos()->attach($todos_tipos);

                        Log::info('Usuário '.$user->id.' liberado inscrição regional centrooeste '.date('Y').'');
                    }
                }
