...
class RegionalNorteController extends Controller
{
    ...
    public function formregionalnorte()
    {
        $user = $this->usuario();
        $regiao = 5;

        return view('regionais.norte.form', compact('user', 'regiao'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($now <= '2022-05-18 00:00:00') {
                ...
                $regional = RegionalNorte::whereUserId($user->id)->first();

                if (empty($regional)) {
                    RegionalNorte::create([
                        'user_id' => $user->id,
                        'regiao' => 'norte',
                ...
                if (Auth::user()->anuidade_2022 && Auth::user()->is_associado) {
                    if (Auth::user()->pago_regional_norte_2022 == false) {
                        $todos_tipos = [0 => 10];
                        $user->todos_tipos()->attach($todos_tipos);

                        Log::info('Usuário '.$user->id.' liberado inscrição regional norte '.date('Y').'');
                    }
                }

                Log::info('User: '.Auth::user()->id.' | Se inscreveu ou atualizou a inscrição no regional Norte: '.json_encode($post));
