            ...
            ->when($request->produto, function ($query) use ($request) {
                $query->where(function ($query) use ($request) {    
                    $query->whereHas('vendas_item', function($query) use ($request) {
                        $query->when($request->produto == 1, function ($query) use ($request) {
                            $query->whereIn('produto_id', [1,2]);
                        });
                        $query->when($request->produto == 2, function ($query) use ($request) {
                            $query->whereIn('produto_id', [1,2]);
                        });
                        $query->when($request->produto == 3, function ($query) use ($request) {
                            $query->whereIn('produto_id', [3,4,5,6]);
                        });
                        $query->when($request->produto == 4, function ($query) use ($request) {
                            $query->whereIn('produto_id', [8,9,10,11]);
                        });
                        $query->when($request->produto == 5, function ($query) use ($request) {
                            $query->whereIn('produto_id', [13,14,15,16]);
                        });
                        $query->when($request->produto == 6, function ($query) use ($request) {
                            $query->whereIn('produto_id', [18,19,20,21]);
                        });
                        $query->when($request->produto == 7, function ($query) use ($request) {
                            $query->whereIn('produto_id', [23,24,25,26]);
                        });
                        $query->when($request->produto == 8, function ($query) use ($request) {
