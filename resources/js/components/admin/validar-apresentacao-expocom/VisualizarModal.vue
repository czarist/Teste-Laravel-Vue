     ...
     <b-modal id="visualizarModal" no-close-on-backdrop>
        ...
        <template>
            <b-alert show variant="danger">
                <h5>ATENÇÃO!</h5>
                <p>
                <strong>Tem certeza que deseja confirmar a apresentação desse trabalho ? </strong> Essa ação é irreversível!</p>
...
export default {
    ...
    methods: {
        ...
        confirmar(selected){          
            ...
            axios.post(`${process.env.MIX_BASE_URL}/validar-apresentacao-expocom/confirmar_apresentacao`, selected).then( res => {
                this.loading = false;
                if(res.status == 200){
