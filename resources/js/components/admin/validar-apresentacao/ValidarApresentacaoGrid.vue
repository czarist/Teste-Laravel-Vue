                        ...
                        <table class="table table-sm table-striped table-hover table-bordered" v-if="user">
                            ...
                            <tbody>
                                <tr v-for="(registro, index) in registros" :key="index">
                                    ...
                                    <td class="align-middle text-center " style="font-size: 11px !important;">
                                        <div v-if="registro && registro.submissao && registro.submissao.apresentacao == 1 && regiao_search == 'Norte'">
                                            <span class="badge badge-success">
                                                <i class="fa fa-check"></i>
                                            </span>
                                        </div>
                                        <div v-else-if="registro && registro.submissao && registro.submissao.apresentacao == 1 && regiao_search == 'Nordeste'">
                                            <span class="badge badge-success">
                                                <i class="fa fa-check"></i>
                                            </span>
                                        </div>
                                        <div v-else-if="registro && registro.submissao && registro.submissao.apresentacao == 1 && regiao_search == 'Sul'">
                                            <span class="badge badge-success">
                                                <i class="fa fa-check"></i>
                                            </span>
                                        </div>
                                        <div v-else-if="registro && registro.submissao && registro.submissao.apresentacao == 1 && regiao_search == 'Sudeste'">
                                            <span class="badge badge-success">
                                                <i class="fa fa-check"></i>
                                            </span>
                                        </div>
                                        <div v-else-if="registro && registro.submissao && registro.submissao.apresentacao == 1 && regiao_search == 'Centro-Oeste'">
                                            <span class="badge badge-success">
                                                <i class="fa fa-check"></i>
                                            </span>
                                        </div>
...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            registros: {
                handler:function(newVal) {
                    newVal.forEach(registro => {
                        if(registro && registro.regional_nordeste && registro.regional_nordeste.submissao_mesa){
                            registro.submissao = registro.regional_nordeste.submissao_mesa
                        }
                        if(registro && registro.regional_nordeste && registro.regional_nordeste.submissao_dt){
                            registro.submissao = registro.regional_nordeste.submissao_dt
                        }
                        if(registro && registro.regional_nordeste && registro.regional_nordeste.submissao_junior){
                            registro.submissao = registro.regional_nordeste.submissao_junior
                        }

                        if(registro && registro.regional_suldeste && registro.regional_suldeste.submissao_mesa){
                            registro.submissao = registro.regional_suldeste.submissao_mesa
                        }
                        if(registro && registro.regional_suldeste && registro.regional_suldeste.submissao_dt){
                            registro.submissao = registro.regional_suldeste.submissao_dt
                        }
                        if(registro && registro.regional_suldeste && registro.regional_suldeste.submissao_junior){
                            registro.submissao = registro.regional_suldeste.submissao_junior
                        }

                        if(registro && registro.regional_norte && registro.regional_norte.submissao_mesa){
                            registro.submissao = registro.regional_norte.submissao_mesa
                        }
                        if(registro && registro.regional_norte && registro.regional_norte.submissao_dt){
                            registro.submissao = registro.regional_norte.submissao_dt
                        }
                        if(registro && registro.regional_norte && registro.regional_norte.submissao_junior){
                            registro.submissao = registro.regional_norte.submissao_junior
                        }

                        if(registro && registro.regional_sul && registro.regional_sul.submissao_mesa){
                            registro.submissao = registro.regional_sul.submissao_mesa
                        }
                        if(registro && registro.regional_sul && registro.regional_sul.submissao_dt){
                            registro.submissao = registro.regional_sul.submissao_dt
                        }
                        if(registro && registro.regional_sul && registro.regional_sul.submissao_junior){
                            registro.submissao = registro.regional_sul.submissao_junior
                        }

                        if(registro && registro.regional_centrooeste && registro.regional_centrooeste.submissao_mesa){
                            registro.submissao = registro.regional_centrooeste.submissao_mesa
                        }
                        if(registro && registro.regional_centrooeste && registro.regional_centrooeste.submissao_dt){
                            registro.submissao = registro.regional_centrooeste.submissao_dt
                        }
                        if(registro && registro.regional_centrooeste && registro.regional_centrooeste.submissao_junior){
                            registro.submissao = registro.regional_centrooeste.submissao_junior
                        }
                    })        
                },
                deep:true
        ...
        methods: {
            find_dt(registro){
                if(registro && registro.submissao && registro.submissao.dt){

                    if(registro && registro.submissao.tipo == "Mesa" || registro.submissao.tipo == "Divisões Temáticas"){
                        let selectedDt =  this.divisoes_tematicas.find(dt => dt.id === registro.submissao.dt)
                        let dt = selectedDt ? selectedDt.dt : ''
                        let dt_descricao = selectedDt ? selectedDt.descricao : ''
                        let retorno = dt
                        return retorno ? retorno : "Sem Informação"

                    }

                    if(registro && registro.submissao.tipo == "Intercom Júnior"){
                        let selectedDt =  this.divisoes_tematicas_jr.find(dt => dt.id === registro.submissao.dt)
                        let dt = selectedDt ? selectedDt.dt : ''
                        let dt_descricao = selectedDt ? selectedDt.descricao : ''
                        let retorno = dt
                        return retorno ? retorno : "Sem Informação"
            ...
            ConfirmarApresentacao(registro){
                ...
                if(this.regiao_search == "Nordeste"){
                    ...
                    var submissao = registro.submissao.id
                    var regional = registro.regional_nordeste.id
                    var validado = registro.submissao.apresentacao
                }
            ...
            getCoordenador(){
                ...
                $.ajax({
                    ...
                    success: (res) => {
                        ...
                        this.regiao_search = this.coordenador ? this.coordenador.regiao : "Nordeste";
                    },
                    error: (res) => {
                        console.log(res)
