<template>
    <b-modal id="instituicaoModal" no-close-on-backdrop>
        <template #modal-header="{ close }">
            <h5 v-if="post.id">Editar instituicao</h5>
            <h5 v-else>Cadastrar instituicao</h5>
            <b-button size="sm" variant="outline-danger" @click="close()">x</b-button>
        </template>
        <template>
            <b-form-group label="Instituicao" label-class="font-weight-bold">
                <b-form-input
            ...
            save() {
                this.$validator.validate().then((valid) => {
                    if(valid) {
                        ...
                        this.loading = true
                        axios.post(`${process.env.MIX_BASE_URL}/config/instituicao${this.url}`, this.post).then(res => {
                            this.clear()
                            if(res.status == 201) {
                                this.loading = false
