<template>
    <b-modal id="titulacaoModal" no-close-on-backdrop>
        <template #modal-header="{ close }">
            <h5 v-if="post.id">Editar titulacao</h5>
            <h5 v-else>Cadastrar titulacao</h5>
            <b-button size="sm" variant="outline-danger" @click="close()">x</b-button>
        </template>
        <template>
            <b-form-group label="Titulacao" label-class="font-weight-bold">
                <b-form-input
            ...
            save() {
                this.$validator.validate().then((valid) => {
                    if(valid) {
                        ...
                        this.loading = true
                        axios.post(`${process.env.MIX_BASE_URL}/config/titulacao${this.url}`, this.post).then(res => {
                            this.clear()
                            if(res.status == 201) {
                                this.loading = false
