							...
							<b-card-text>
								<div class="container">
									<div class="section-head style-3 text-center z mb-3">
										<h2 class="title">Checkout <img :src="baseUrl + '/images/pagseguro.png'" /></h2>
					...
					<div class="container">
						...
						<div class="hidden">
							<form method="post" accept-charset="utf-8" name="formPagamentoCartaoCredito" id="formPagamentoCartaoCredito" action="" >
								...
								<input type="hidden" name="produto" id="produto" :value="form && form.produto ? form.produto.id : null" />
							</form>
						</div>
					</div>
				</b-card-text>
  ...
  watch: {
    selectedPagar() {
		if (this.selectedPagar) {
			...
			this.form.valorParcela = this.produtos.find((produto) => produto.id == this.selectedPagar.taxa_inscricao.id).valor;
			this.form.parcelas = 1;
		}
      if (this.form.anuidade2022 == 1) {
        this.amount = this.produtos.find((produto) => produto.id == this.selectedPagar.taxa_inscricao.id).valor;
        this.form.produto = this.produtos.find((produto) => produto.id == this.selectedPagar.taxa_inscricao.id);
      }
	...
	methods: {
		...
		PagSeguroDirectPayment.getBrand({
			...
			$(".bandeira-cartao").html(
				"<img src='https://stc.pagseguro.uol.com.br/public/img/payment-methods-flags/68x30/" +
				imgBand +
				".png'>"
			);
			...
			$("#retorno_texto").html(
				"Número do cartão invalido, por favor verifique e tente novamente."
			);
		...
		async pagarBoleto() {
			...
			await PagSeguroDirectPayment.onSenderHashReady((retorno) => {
				...
				if (retorno.status == "error") {
					...
					this.message(
						"Erro",
						"O Pagseguro pode estar com lentidão ou instabilidade, clique no botão VOLTAR e tente novamente. Caso ja tenha feito esse processo mais de duas vezes, tente novamente em alguns minutos",
						"error"
					);
