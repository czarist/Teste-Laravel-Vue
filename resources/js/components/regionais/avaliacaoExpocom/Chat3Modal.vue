<template>
    <b-modal id="modalChat3" size="xl" no-close-on-backdrop>
        <template #modal-header="{ close }">
            <h5>MENSAGENS  PARA O AVALIADOR 3</h5>
        ...
        <template>
            ...
            <div class="modal-body scroll pb-0" ref="scroll">
                <div v-for="(mensagem, indexMensagem) in mensagens" :key="indexMensagem">
                    ...
                    <span 
                        v-if="selectedChat.avaliacao.avaliador_3 == mensagem.avaliador_id ||
                        selectedChat.coordenador_id && mensagem.send_avaliador == 3
                        "
                        :class="
                    ...
                    >
                        <div class="d-flex justify-content-between" >
                            ...
                            <span v-if="mensagem && mensagem.avaliador"> 
                                <b>{{ mensagem && mensagem.created_at | momentFullDate }} - {{ mensagem && mensagem.avaliador ? 'Avaliador' : 'Avaliador' }}</b> 
                            </span>
    ...
    methods: {
        ...
        send(){
            this.$validator.validateAll().then(valid => {
                if (valid) {
                    this.loading = true;
                    axios.post(`${this.baseUrl}/coordenador/expocom/send/message`, this.selectedChat).then( res =>{
                        this.loading = false;
                        if(res.status == 201){
