      ...
      export default {
        ...
        methods: {
            liveCountDown: function() {
                this.totalRemainCount = this.limitmaxCount - this.post.justificativa_coordenador.length;
                ...
                await this.$validator.validateAll().then((valid) => {
                    if(valid) {
                        if(this.generateErr == true){
                            ...
                            setTimeout(() => {
                                axios.post(`${process.env.MIX_BASE_URL}/avaliacaoexpocom/coordenador/save`, this.post).then( res => {
