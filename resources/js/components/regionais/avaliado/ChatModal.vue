<template>
    <b-modal id="modalChat" size="xl" no-close-on-backdrop>
        ...
        <template>
            ...
            <div class="modal-body scroll pb-0" ref="scroll">
                <div v-for="(mensagem, indexMensagem) in mensagens" :key="indexMensagem">
                    <span 
                        v-if="mensagem && !mensagem.avaliador_id"
                        :class="mensagem && mensagem.coordenador  && mensagem.coordenador.user && mensagem.coordenador.user.id == user.id  ? `alert alert-warning float-right mb-2 w-100` : `alert alert-info float-left mb-2 w-100`" 
                        style="max-width:87% !important; border-radius:8px !important;" 
                    >
                            
                        <div class="d-flex justify-content-between" >
                            <span v-if="mensagem && mensagem.coordenador"> 
                                <b>{{ mensagem && mensagem.created_at | momentFullDate }} - {{ mensagem && mensagem.coordenador  && mensagem.coordenador.user ? 'Coordenador' : 'Coordenador' }}</b> 
                            </span>

                            <span v-if="mensagem && mensagem.avaliador"> 
                                <b>{{ mensagem && mensagem.created_at | momentFullDate }} - {{ mensagem && mensagem.avaliador && mensagem.avaliador.user ? 'Avaliador' : 'Avaliador' }}</b> 
                            </span>

                            <span v-if="mensagem && mensagem.avaliado"> 
                                <b>{{ mensagem && mensagem.created_at | momentFullDate }} - {{ mensagem && mensagem.avaliado && mensagem.avaliado.id == user.id ? mensagem.avaliado.name : "Autor"}}</b> 
                            </span>
    ...
    methods: {
        ...
        send(){
            this.$validator.validateAll().then(valid => {
                if (valid) {
                    this.loading = true;
                    axios.post(`${this.baseUrl}/coordenador/send/message`, this.selectedChat).then( res =>{
                        this.loading = false;
                        if(res.status == 201){
